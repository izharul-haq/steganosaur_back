...
def encrypt(_input: bytes, key: bytes) -> list[int]:
    '''Encrypt given input with given key using
    modified RC4 alogrithm.'''
    ...
    for idx in range(0, len(_input)):
        ...
        res.append(keystream ^ _input[idx])

    return res


def decrypt(_input: list[int], key: bytes) -> bytes:
    '''Decrypt given input with given key using
    modified RC4 alogrithm.'''
